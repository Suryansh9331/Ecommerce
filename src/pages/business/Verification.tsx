...
const Verification: React.FC = () => {
  ...
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [selectedCountry, setSelectedCountry] = useState('IN'); // lock India
  const [countryConfig, setCountryConfig] = useState<CountryConfig | null>(null);
  const [supportedCountries, setSupportedCountries] = useState<Array<{ code: string; name: string }>>([]);
  const [isLoading, setIsLoading] = useState(true);
  ...
  useEffect(() => {
    const fetchCountryConfig = async () => {
      ...
      try {
        ...
        // Force only PAN Card and Aadhar to be required; others optional
        const transformedRequiredDocs = (config.required_documents || []).map(
        ...
        const newDocuments: { [key: string]: DocumentUpload } = {};
        if (transformedConfig.required_documents) {
          transformedConfig.required_documents.forEach(
            (doc: { type: string; name: string; required: boolean }) => {
              newDocuments[doc.type] = {
                id: doc.type,
                file: null,
  ...
  const validateField = (name: string, value: string) => {
    if (!countryConfig || !countryConfig.field_validations) return;

    const errors = { ...validationErrors };
    const validationRule = countryConfig.field_validations[name];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Only these are required from user input
    const requiredFields = ['panNumber', 'accountNumber', 'bankName', 'ifscCode'];
    ...
    requiredFields.forEach(field => {
      ...
      if (!value || value.trim() === '') {
        currentValidationErrors[field as keyof ValidationErrors] =
    ...
    // Validate required documents: only PAN & Aadhar remain required
    let allRequiredDocsUploaded = true;
    Object.values(documents).forEach(doc => {
      if (doc.required && doc.status !== 'uploaded') {
        allRequiredDocsUploaded = false;
    ...
    try {
      ...
      const profileData = {
        country_code: selectedCountry || 'IN',

        // visible
        pan_number: businessDetails.panNumber || 'ABCDE1234F',

        // hidden → dummy safe values
        gstin: '22AAAAA0000A1Z5',
        tax_id: 'TAX12345',
        vat_number: 'VAT12345',
        sales_tax_number: 'STX12345',

        bank_account_number: bankDetails.accountNumber || '000111222333',
        bank_name: bankDetails.bankName || 'Default Bank',
        bank_branch: bankDetails.bankBranch || 'Main Branch',
        bank_ifsc_code: bankDetails.ifscCode || 'SBIN0000001',

        bank_swift_code: bankDetails.swiftCode || 'SBINUS33',
        bank_routing_number: bankDetails.routingNumber || '123456789',
        bank_iban: bankDetails.iban || 'GB33BUKB20201555555555'
      };
          ...
          */}
          ...
          <div className="mb-8">
            ...
            <div className="space-y-6">
              ...
              {/* HIDDEN EXTRA BUSINESS FIELDS */}
              {/*
              <div>GSTIN field...</div>
              <div>Tax ID field...</div>
              <div>VAT Number field...</div>
              <div>Sales Tax Number field...</div>
              */}
            </div>
          </div>
          ...
          <div className="mb-8">
            ...
            <div className="space-y-6">
              ...
              {/* Extra bank fields hidden in UI but still in payload via dummy/default */}
              {/*
              <div>swiftCode</div>
            ...
            <div className="space-y-6">
              {countryConfig && Object.keys(documents).length > 0 ? (
                Object.values(documents)
                  .filter(doc => doc.documentType === 'pan_card' || doc.documentType === 'aadhar')
                  .map(doc => (
                    <div key={doc.id} className="bg-white p-4 rounded-lg shadow">
                      <label className="block mb-2 font-medium text-gray-700">
                        {doc.name}
                      ...
                      {doc.status === 'uploaded' && doc.fileUrl ? (
                        <div className="mt-1">
                          <div className="flex items-center p-2 bg-green-50 border border-green-200 rounded-md">
                            <CheckCircleIcon className="h-5 w-5 mr-2 text-green-600 flex-shrink-0" />
                            <p
                              className="text-sm text-green-700 font-medium truncate flex-grow"
                              title={doc.file?.name || 'Document uploaded'}
                            >
                              {doc.file?.name
                                ? doc.file.name.length > 30
                                  ? doc.file.name.substring(0, 27) + '...'
                                  : doc.file.name
                                : 'Uploaded'}
                            </p>
                          </div>
                ...
                <p className="text-gray-500 text-sm">
                  No documents are currently required for {countryConfig.country_name}.
                </p>
              ) : (
                <p className="text-gray-500 text-sm">Select a country to view document requirements.</p>
              )}
          ...
          <div className="flex justify-center mt-10 pt-6 border-t border-gray-200">
            <button
              type="submit"
              disabled={isSubmitting || (isLoading && !countryConfig)}
              className={`
                px-8 py-3 rounded-md text-white font-semibold text-lg
                ${
